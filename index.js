...
// search get request
app.get('/search/difficulty/:value', (req, res) => {
    let plans = planController.getPlansByDifficulty(parseInt(req.params.value))
    if (plans) {
        res.send(plans)
    } else {
        res.send('ERROR: Could not find workout plans of this difficulty')
    }
});

app.get('/search/name/:value', (req, res) => {
    let plans = planController.getPlansByName(parseInt(req.params.value))
    if (plans) {
        res.send(plans)
    } else {
        res.send('ERROR: Could not find workout plans of this name')
    }
});
